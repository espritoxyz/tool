...
fun generateTests(
    ...
    val wrapperFolder = projectPath.resolve("wrappers")
    val testsFolder = projectPath.resolve("tests")
...
private fun extractAddress(bits: String): String? {
    ...
    if (bits.length < 267 || bits.take(2) != "10") {
        return null
    }
...
private fun renderTestBodies(name: String, tests: List<TvmSymbolicTest>, ident: String): String {
    val testBodies = tests.map { test ->
        ...
        """
            ...
            $ident        await contract.initializeContract(blockchain, ${contractCurrencyValue}n)
            $ident  
            $ident        const sentMessageResult = await contract.internal(
            $ident            blockchain,
            $ident            from,
            $ident            msgBody,
            $ident            ${msgCurrencyValue}n,
            $ident            bounce,
            $ident            bounced
            $ident        )
